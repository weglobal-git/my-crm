...
| `/contact` | SSR list + ContactView local state, public events; SaleTargetTab active-only | authorized account/person delta เมื่อกำลังดู; SaleTargetTab ไม่สร้าง channel เพิ่ม | initial/filter/page fetch; SaleTargetTab โหลดเมื่อ active tab เท่านั้น | CRUD ทั่วไปใช้ local feedback; notify เมื่อมี assignment/action ชัดเจน |
