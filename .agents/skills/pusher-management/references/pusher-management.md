...
| `/contact` | Full-width AccountCardList + ContactView orchestration, `private-contacts` channel; detail on pointer/focus intent; SaleTargetTab active-only | authorized account delta (`RATING_CHANGE`, `DETAILS_CHANGE`, `STATUS_CHANGE`, `COMPANY_CREATED`) พร้อม revision token และ mutationId ข้าม echo; SaleTargetTab ไม่สร้าง channel เพิ่ม | initial SSR card DTO + user-scoped SWR filter cache; infinite scroll; targeted recovery เมื่อ reconnect; detail โหลดเมื่อเปิด drawer | CRUD/rating ใช้ optimistic feedback + revision order; ไม่ broadcast sensitive person data; notify เฉพาะ workflow ที่ต้องการ action ชัดเจน |
...
| P3 Verification & Deployment | Automated test suites & deployment readiness | **เสร็จสมบูรณ์ระดับ codebase:** ผ่าน verify:pipeline ทั้ง 60 tests (0 failures, 0 TypeScript errors) รวม regression ของ initial-hidden lifecycle และ deferred shared-channel acquisition; โค้ดพร้อม deploy สู่ staging/production เพื่อสังเกตการณ์ steady-state connections บน Pusher Dashboard |
