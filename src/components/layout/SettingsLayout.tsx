...
export function SettingsSidebar({ children, title, searchInput }: { children: React.ReactNode, title?: string, searchInput?: React.ReactNode }) {
  return (
    <div className="w-full md:w-64 lg:w-72 shrink-0 flex flex-col gap-4">
      {title && <h2 className="text-2xl font-bold text-slate-100 px-2">{title}</h2>}
...
export function SettingsSidebarItem({ 
  ...
  return (
    ...
    >
      <div className={`mt-0.5 p-1.5 rounded-lg transition-colors shrink-0 flex items-center justify-center ${isActive ? 'bg-black text-white' : 'text-slate-400 bg-[#3A3B3C] group-hover/item:bg-[#4E4F50] group-hover/item:text-slate-200'}`}>
    ...
    <div className="flex-1 w-full flex flex-col gap-6">
      {(title || action) && (
        <div className="flex items-end justify-between pb-4">
          {title && <h3 className="text-2xl font-bold text-slate-100 px-2">{title}</h3>}
...
export function SettingsGroup({ children, label }: { children: React.ReactNode, label?: string }) {
  return (
    <div className="flex flex-col gap-2">
      {label && <h4 className="text-xs font-semibold text-slate-400 px-4 uppercase tracking-wider">{label}</h4>}
      <div className="bg-[#3A3B3C] rounded-[14px] border border-[#4E4F50] overflow-hidden flex flex-col">
        {children}
      </div>
    </div>
  );
...
export function SettingsRow({ children, className = "" }: { children: React.ReactNode, className?: string }) {
  return (
    <div className={`flex items-center justify-between p-4 border-b border-[#4E4F50] last:border-b-0 ${className}`}>
      {children}
    </div>
  );
}
