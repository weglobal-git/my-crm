...
export function DatePicker({ value, onChange, placeholder = "mm/dd/yyyy", className = "", allowPastDates = false }: DatePickerProps) {
  ...
  return (
    <div className="relative w-full" ref={popupRef}>
      <div 
        className={`w-full bg-[#252728] border border-[#4E4F50] rounded-xl px-3 py-2.5 text-slate-100 text-xs cursor-pointer flex items-center justify-between transition-colors hover:border-[#6b6c6d] ${isOpen ? 'border-[#C7F33C]' : ''} ${className}`}
