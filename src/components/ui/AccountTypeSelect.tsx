...
export const ACCOUNT_TYPE_OPTIONS: { value: ContactType; label: string }[] = [
...
export function AccountTypeSelect({
  ...
  const handleSelect = (val: ContactType) => {
    onChange(val);
    closeDropdown();
  };
  ...
  return (
    <div ref={containerRef} className={`relative w-full ${className}`}>
      ...
      <button
        ...
        className={`w-full bg-[#252728] rounded-xl px-3.5 py-2 text-xs text-slate-100 border border-[#4E4F50]/40 hover:border-[#4E4F50] focus:outline-none focus:ring-1 focus:ring-[#C7F33C] transition-colors flex items-center justify-between cursor-pointer ${
