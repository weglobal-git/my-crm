...
export function UserSearchDropdown({ 
  ...
  return (
    <div ref={dropdownRef} className={`absolute top-full ${alignmentClass} mt-2 w-80 bg-[#3A3B3C] rounded-xl border border-[#4E4F50] z-30 p-2 animate-fade-in-up`}>
      <div className="mb-2">
        <input 
          type="text" 
      ...
      <div className="max-h-40 overflow-y-auto custom-scrollbar flex flex-col gap-1">
        ...
        {filteredUsers.length === 0 && (
          <div className="p-2 text-center text-xs text-slate-500">No users found.</div>
        )}
      </div>
    </div>
  );
