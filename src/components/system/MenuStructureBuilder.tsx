    ...
    <div className="flex flex-col relative min-h-[500px]">
        ...
        <div className="space-y-12">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Settings className="w-5 h-5 text-indigo-500" /> User Tools
            </h3>
            <SortableContext items={userTools.map(m => m.id)} strategy={rectSortingStrategy}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <button
                  ...
                  className="border-2 border-dashed border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px] hover:border-indigo-300 bg-white hover:bg-slate-50 transition-colors text-slate-400 hover:text-indigo-500 group"
                >
                  <div className="p-3 bg-white rounded-full border border-slate-200 mb-3 group-hover:border-indigo-200 group-hover:text-indigo-500 transition-colors">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Settings className="w-5 h-5 text-rose-500" /> Admin Tools
            </h3>
      ...
      {editModal.isOpen && (
        <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] border border-slate-100 p-8 w-full max-w-md shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-slate-800">{editModal.id ? "Edit Category" : "New Category"}</h2>
              <button onClick={() => setEditModal({ isOpen: false, label: '', icon: 'Settings', description: '', isSubMenu: false })} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-4 mb-8">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
            ...
            <div className="flex justify-end gap-3">
              <button 
                ...
                className="px-5 py-2.5 rounded-full text-slate-600 font-semibold hover:bg-slate-50 transition-colors"
              >
                Cancel
              </button>
              <button 
                ...
                disabled={isSaving}
                className="px-5 py-2.5 rounded-full bg-slate-900 text-white font-semibold hover:bg-black transition-colors disabled:opacity-50"
              >
...
function SortableMainMenuBucket({ mainMenu, subMenus, onEdit, onEditSub, onDelete, isOverlay }: { mainMenu: MenuItemType; subMenus: MenuItemType[]; onEdit: () => void; onEditSub: (sub: MenuItemType) => void; onDelete: () => void; isOverlay?: boolean }) {
  ...
  return (
    <div 
      ...
      style={style}
      className={`
        bg-white rounded-2xl border p-4 flex flex-col h-full min-h-[300px] transition-colors relative group
        ${isOver ? 'border-indigo-400 bg-indigo-50/30' : 'border-slate-100'}
        ${isOverlay ? 'scale-105 rotate-1 z-50 bg-white' : ''}
      `}
    >
      {/* Actions */}
      <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 z-10">
        <button onClick={onEdit} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-md transition-colors"><Edit2 className="w-4 h-4" /></button>
        <button onClick={onDelete} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-white rounded-md transition-colors"><Trash2 className="w-4 h-4" /></button>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6 px-2 pr-16 group/header cursor-grab active:cursor-grabbing" {...attributes} {...listeners}>
        <GripHorizontal className="w-5 h-5 text-slate-300 group-hover/header:text-indigo-400 transition-colors" />
        {Icon && <div className="p-2 bg-white rounded-lg border border-slate-200 text-slate-600"><Icon className="w-5 h-5" /></div>}
        <h3 className="font-bold text-slate-800 tracking-tight">{mainMenu.label}</h3>
      </div>
...
function SubMenuCard({ menu, isDragging, isOverlay, onEdit }: { menu: MenuItemType; isDragging?: boolean; isOverlay?: boolean; onEdit?: () => void }) {
  ...
  return (
    <div className={`
      relative bg-white rounded-xl border border-slate-200 p-3 flex items-center gap-3 group
      ${isDragging ? 'border-dashed bg-slate-50' : 'hover:border-indigo-200 transition-colors cursor-grab active:cursor-grabbing'}
      ${isOverlay ? 'scale-105 rotate-2 z-50 bg-white/90 backdrop-blur' : ''}
    `}>
      <div className="text-slate-300 group-hover:text-indigo-400 transition-colors">
        <GripVertical className="w-4 h-4" />
      </div>
      {Icon && <Icon className="w-4 h-4 text-slate-500" />}
      <span className="font-semibold text-slate-700 text-sm flex-1">{menu.label}</span>
      {onEdit && (
        <div className="absolute right-2 opacity-0 group-hover:opacity-100 flex gap-1 bg-white p-1 rounded-md shadow-sm border border-slate-100">
          <button onPointerDown={(e) => { e.stopPropagation(); onEdit(); }} className="p-1 text-slate-400 hover:text-indigo-600 transition-colors">
