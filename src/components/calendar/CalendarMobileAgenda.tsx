...
export function CalendarMobileAgenda({ date, items, onEventClick }: { date: Date; items: CalendarMonthItemDTO[]; onEventClick: (item: CalendarMonthItemDTO) => void }) {
  const [selectedDealItem, setSelectedDealItem] = useState<CalendarMonthItemDTO | null>(null);
  return <section className="min-h-0 flex-1 overflow-y-auto border-t border-[#3A3B3C] pt-3" aria-label="Selected day items">
    ...
    {selectedDealItem && (
      <SaleDealPanel
        opportunityId={selectedDealItem.sourceId}
        initialData={{
          opportunityId: selectedDealItem.sourceId,
          accountName: selectedDealItem.accountName || null,
          topicName: selectedDealItem.title,
          totalValue: null,
          currency: 'THB',
          reserveId: null,
          invoiceNumber: null,
          goodsReadyDate: selectedDealItem.sourceType === 'DEAL_GOODS_READY' ? selectedDealItem.startAt : null,
          goodsLoadingDate: selectedDealItem.sourceType === 'DEAL_GOODS_LOADING' ? selectedDealItem.startAt : null,
          status: 'OPEN',
          revision: 0,
          canEdit: Boolean(selectedDealItem.canEdit),
        }}
        presentation="sheet"
        onClose={() => setSelectedDealItem(null)}
      />
    )}
  </section>;
}
